...
def energy_terms(
    ...
    """Returns a function that performs a time step of Energy Eq."""
    del grid  # unused
    ...
    def _explicit_terms(rhoE, *args):
        drhoE_dt  = convect(rhoE, *args)
        drhoE_dt += diffuse(rhoE, *args)
        drhoE_dt += forcing(rhoE, *args)
        return drhoE_dt

    def explicit_terms_with_same_bcs(rhoE, *args):
        drhoE_dt = _explicit_terms(rhoE, *args)
        return grids.GridVariable(drhoE_dt, rhoE.bc) 
...
def explicit_energy_eq(
  ...
  ode = time_stepping.EnergyODE(explicit_terms, properties)
