...
def diffuse(rhov, v, state):
    ...
    # gradV = tuple(
    #         tuple(
    #             base.grids.GridVariable(fd.central_difference(u, a)
    #             , bc=col.boundaries.ConstantBoundaryConditions((('dirichlet', 'dirichlet'),('dirichlet','dirichlet')),
    #                                                         [(jnp.take(fd.backward_difference(u, a).data, indices=jnp.array([ 0]), axis=_a),
    #                                                           jnp.take(fd.forward_difference( u, a).data, indices=jnp.array([-1]), axis=_a)) 
    #                                                           for _a in range(ndim)]
    #                                                         ))
    #         for a in range(ndim))
    #         for u in v)
    # return (properties['dyna_viscosity']*(fd.central_difference(gradV[0][0], 0) + fd.central_difference(gradV[0][1], 1)), 
    #         properties['dyna_viscosity']*(fd.central_difference(gradV[1][0], 0) + fd.central_difference(gradV[1][1], 1)))
    ...
    # tau =   tuple(
    #         tuple(
    #             base.grids.GridVariable(gradV[i][j].array + gradV[j][i].array, 
    #                                     bc=col.boundaries.ConstantBoundaryConditions(
    #                                         (('dirichlet', 'dirichlet'),('dirichlet','dirichlet')),
    #                                         (tree_math.Vector(gradV[i][j].bc._values)
    #                                          +tree_math.Vector(gradV[j][i].bc._values)).tree))
    #         for j in range(ndim))
    #         for i in range(ndim))
