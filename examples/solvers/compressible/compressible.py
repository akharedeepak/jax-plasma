...
def run_collocated(size, seed=0, inner_steps=100, outer_steps=10):
    ...
    def convect(rhov, v, state):
        return tuple(advection.advect_linear(rhou, v, dt) for rhou in rhov)
    ...
    def diff_stress(v):
        ...
        tauv = properties['dyna_viscosity']*grids.GridArrayTensor(tau.data) * grids.GridArrayTensor((u.data for u in v))
        # div_tau =   tuple(
        #             properties['dyna_viscosity']*sum(fd.central_difference(tau[i][j], j) for j in range(len(v)))
        #             for i in range(len(v)))
        return grids.GridVariable(tauv, bc=boundaries.neumann_boundary_conditions( ndim=len(v), bc_vals=((0,0),(0,0))) )

    def diffuse(rhoE, T, state):
        return properties['thermal_cond'] * fd.laplacian(T)# + diff_stress(T)
